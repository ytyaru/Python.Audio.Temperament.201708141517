...
'''
    ...
    for hz in [432, 440, 444]:
        ...
        """
        t = MusicTheory.temperament.EqualTemperament.EqualTemperament()
        table_body += '平均律' + '|' + tm.get_audios_TemperamentScales(t, hz) + '|' + tm.get_audios_TemperamentChords(t, hz, 'power') + '|' + tm.get_audios_TemperamentChords(t, hz, 'major') + '\n'
        t = MusicTheory.temperament.JustIntonation.JustIntonation()
        table_body += '平均律' + '|' + tm.get_audios_TemperamentScales(t, hz) + '|' + tm.get_audios_TemperamentChords(t, hz, 'power') + '|' + tm.get_audios_TemperamentChords(t, hz, 'major') + '\n'
        t = MusicTheory.temperament.PythagoreanTuning.PythagoreanTuning()
        table_body += '平均律' + '|' + tm.get_audios_TemperamentScales(t, hz) + '|' + tm.get_audios_TemperamentChords(t, hz, 'power') + '|' + tm.get_audios_TemperamentChords(t, hz, 'major') + '\n'
        """
...
#            elif isinstance (temperament, MusicTheory.temperament.PythagoreanTuning.PythagoreanTuning): name = 'ピタゴラス音律'
    ...
    for hz in [432, 440, 444]:
        ...
        for temperament in [
            ...
            table_body += name + '|' + tm.get_audios_TemperamentScales(temperament, hz)
        table_body += '\n'
    body += table_body + '\n'

    table_body = '## 和音' + '\n\n'
    for hz in [432, 440, 444]:
        for chord in ['power', 'major']:
            if chord == 'power': chord_name = 'パワーコード'
            elif chord == 'major': chord_name = 'メジャーコード'
#            table_body += f'### {chord_name}' + '\n\n'
#            table_body += f'#### {hz}Hz' + '\n\n' + '音律|play' + '\n' + '----|----' + '\n'
            table_body += f'### {hz}Hz {chord_name}' + '\n\n' + '音律|play' + '\n' + '----|----' + '\n'
            for temperament in [
                MusicTheory.temperament.EqualTemperament.EqualTemperament(), 
                MusicTheory.temperament.JustIntonation.JustIntonation(), 
                MusicTheory.temperament.PythagoreanTuning.PythagoreanTuning()]:
                ...
                table_body += name + '|' + tm.get_audios_TemperamentChords(temperament, hz, chord)
            table_body += '\n'
        table_body += '\n'
    body += table_body + '\n'
    """
